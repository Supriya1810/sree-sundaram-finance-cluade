...
export default function GoldCanvas3D() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gradBg.addColorStop(0, 'rgba(212, 160, 23, 0.07)');
      gradBg.addColorStop(0.5, 'rgba(43, 18, 10, 0)');
      gradBg.addColorStop(1, 'rgba(17, 17, 17, 0)');
      ...
      ringRadii.forEach((r, idx) => {
        ctx.beginPath();
        ctx.ellipse(0, 0, r, r * 0.35, globalRotation * (0.2 + idx * 0.1), 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(212, 160, 23, ${0.06 + 0.06 * (3 - idx)})`;
      ...
      particles.forEach((p) => {
        p.angle += p.speed;
        const wobble = Math.sin(globalRotation * 2 + p.angle * 3) * p.wobbleRange;
        const currentRadius = p.orbitRadius + Math.sin(globalRotation * p.wobbleSpeed) * 10;
        ...
        particleGrad.addColorStop(0.3, '#F4C542');
        particleGrad.addColorStop(0.7, '#D4A017');
        particleGrad.addColorStop(1, 'rgba(212, 160, 23, 0)');
  ...
  return (
    <div id="canvas-container" className="absolute inset-0 block w-full h-full select-none pointer-events-none z-0 overflow-hidden">
